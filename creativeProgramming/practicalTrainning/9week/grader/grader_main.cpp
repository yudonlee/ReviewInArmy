...
int main() {
	...
	const int total_credit = subject1.credit() + subject2.credit() + subject3.credit() +
		subject4.credit();
	string cmd;
	...
	while (cmd != "quit") {
		...
		if (cmd == "eval") {
			...
			cout << name << " " << grade1 << " " << grade2 << " " << grade3 << " " << grade4 <<
				endl;
			double average_grade = (GetNumberGrade(grade1) * subject1.credit() +
				GetNumberGrade(grade2) * subject2.credit() + GetNumberGrade(grade3) * subject3.credit() +
